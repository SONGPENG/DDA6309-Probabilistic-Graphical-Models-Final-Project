...
class TCwsPos(nn.Module):
    ...
    def __init__(self, word2id, gram2id, feature2id, labelmap, processor, hpara, args):
        ...
        if args.encoder_type == 'transformer':
            self.private_encoder_cws = Transformer(hidden_size, nhead=8, dim_feedforward=2048, dropout=0.1,
                                                   num_layers=self.num_layers)
            self.private_encoder_pos = Transformer(hidden_size, nhead=8, dim_feedforward=2048, dropout=0.1,
                                                   num_layers=self.num_layers)
        elif args.encoder_type == 'bilstm':
            self.private_encoder_cws = BiLSTM(hidden_size, int(hidden_size / 2), num_layers=self.num_layers)
            self.private_encoder_pos = BiLSTM(hidden_size, int(hidden_size / 2), num_layers=self.num_layers)
        else:
            assert False

        self.project_matrix = torch.nn.Linear(hidden_size, hidden_size)
        self.classifier_d = torch.nn.Linear(hidden_size, 2, bias=False)

        self.loss_fct = CrossEntropyLoss()

        self.dropout = nn.Dropout(p=0.5)  # dropout训练

        self.count_all_sample_cws = 0
        self.count_all_correct_sample_cws = 0

        self.count_all_sample_pos = 0
        self.count_all_correct_sample_pos = 0
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, valid_ids=None,
        ...
        private_output_cws = self.private_encoder_cws(sequence_output)
        private_output_pos = self.private_encoder_pos(sequence_output)

        private_output_cws_maxpolling = torch.max(private_output_cws, -2)[0]
        private_output_pos_maxpolling = torch.max(private_output_pos, -2)[0]

        private_output_cws_project = self.project_matrix(private_output_cws_maxpolling)
        private_output_pos_project = self.project_matrix(private_output_pos_maxpolling)

        private_output_cws_project = self.dropout(private_output_cws_project)
        private_output_pos_project = self.dropout(private_output_pos_project)

        private_output_cws_project = self.tanh(private_output_cws_project)
        private_output_pos_project = self.tanh(private_output_pos_project)

        logits_d_cws = self.classifier_d(private_output_cws_project)
        logits_d_pos = self.classifier_d(private_output_pos_project)

        _, cws_d_predict_list = logits_d_cws.max(1)
        _, pos_d_predict_list = logits_d_cws.max(1)

        cws_d_predict_list = cws_d_predict_list.long().to('cuda')
        pos_d_predict_list = pos_d_predict_list.long().to('cuda')

        cws_d_label = torch.zeros(logits_d_cws.shape[0]).long().to('cuda')
        pos_d_label = torch.ones(logits_d_pos.shape[0]).long().to('cuda')

        correct_cws_d = sum(cws_d_predict_list == cws_d_label)
        correct_pos_d = sum(pos_d_predict_list == pos_d_label)

        loss_cws_d = self.loss_fct(logits_d_cws, cws_d_label)
        loss_pos_d = self.loss_fct(logits_d_pos, pos_d_label)

        self.count_all_sample_cws += logits_d_cws.shape[0]
        self.count_all_sample_pos += logits_d_pos.shape[0]

        self.count_all_correct_sample_cws += correct_cws_d
        self.count_all_correct_sample_pos += correct_pos_d

        # print('private_output_cws', private_output_cws.shape)
        # print('private_output_cws_project', private_output_cws_project.shape)

        private_output_cws_project_2 = private_output_cws_project.unsqueeze(1).repeat(1, private_output_cws.shape[1], 1)
        private_output_pos_project_2 = private_output_pos_project.unsqueeze(1).repeat(1, private_output_pos.shape[1], 1)
        # print('private_output_cws_project_2', private_output_cws_project_2.shape)

        final_private_output_cws = private_output_cws * private_output_cws_project_2
        final_private_output_pos = private_output_pos * private_output_pos_project_2

        conc = torch.cat([final_private_output_cws, final_private_output_pos], dim=2)

        logits = self.classifier(conc)

        total_loss = self.crf.neg_log_likelihood_loss(logits, attention_mask, labels) + 0.01 * (loss_cws_d + loss_pos_d)
    ...
    def get_accuracy_d(self):
        acc_cws = self.count_all_correct_sample_cws / self.count_all_sample_cws
        acc_pos = self.count_all_correct_sample_pos / self.count_all_sample_pos
        return (acc_cws, acc_pos)
    ...
    def load_data(self, data_path, do_predict=False):
        ...
        examples = []
        for i, (
                sentence, label, word_list, syn_feature_list, word_matching_position,
...
class SCwsPos(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, valid_ids=None,
        ...
        conc = torch.cat([private_output_cws, private_output_pos], dim=2)
