...
def pos_evaluate_word_PRF(y_pred, y):
    ...
    pPP = precision_score([y], [y_pred])
    pRR = recall_score([y], [y_pred])
    pFF = f1_score([y], [y_pred])

    return (wP, wR, wF), (pP, pR, pF), (pPP, pRR, pFF)
...
def pos_evaluate_OOV(y_pred_list, y_list, sentence_list, word2id):
    ...
    for y_w, y_p, y_p_w, y_p_p, sentence in zip(y_word_list, y_pos_list, y_pred_word_list, y_pred_pos_list,
                                                sentence_list):
        start = 0
