...
from cwspos_helper import get_word2id, getlabels, request_features_from_stanford, request_features_from_berkeley, \
    berkeley_feature_processor, stanford_feature_processor, get_feature2id
from cwspos_eval import eval_sentence, pos_evaluate_word_PRF, pos_evaluate_OOV
from cwspos_model import TCwsPos, SCwsPos, PCwsPos
import datetime
...
def train(args):
    ...
    log_file_name = './logs/log-' + args.model_name + '-' + now_time
    ...
    hpara = SCwsPos.init_hyper_parameters(args)
    joint_model = SCwsPos(word2id, gram2id, feature2id, label_map, processor, hpara, args)
    ...
    best_pf = -1
    best_poov = -1
    best_pPP, best_pRR, best_pFF = -1, -1, -1

    history = {'epoch': [], 'word_p': [], 'word_r': [], 'word_f': [], 'word_oov': [],
               'pos_p': [], 'pos_r': [], 'pos_f': [], 'pos_oov': [], 'ppos_p': [], 'ppos_r': [], 'ppos_f': []}
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, start_index in enumerate(tqdm(range(0, len(train_examples), args.train_batch_size))):
                ...
                loss, _ = joint_model(input_ids, segment_ids, input_mask, label_ids, valid_ids, l_mask, word_ids,
                                      feature_ids, word_matching_matrix, word_matching_matrix, ngram_ids,
                                      ngram_positions)
                if n_gpu > 1:
            ...
            if args.local_rank == -1 or torch.distributed.get_rank() == 0:
                ...
                (wp, wr, wf), (pp, pr, pf), (pPP, pRR, pFF) = pos_evaluate_word_PRF(y_pred_all, y_true_all)
                ...
                history['ppos_p'].append(pPP)
                history['ppos_r'].append(pRR)
                history['ppos_f'].append(pFF)
                ...
                logger.info("Epoch: %d,  ppos P: %f,  ppos R: %f,  ppos F: %f,  ppos OOV: %f",
                            epoch + 1, pPP, pRR, pFF, 123321)
                logger.info("=======entity level========")
                ...
                if pf > best_pf:
                    ...
                    if args.model_name:
                        ...
                        output_model_dir = path.join('./models', args.model_name)
                        if not os.path.exists(output_model_dir):
                            os.mkdir(output_model_dir)
                        ...
                        with open(os.path.join(output_model_dir, 'POS_result.txt'), "w") as writer:
                            ...
                            writer.write("Epoch: %d,  pos P: %f,  pos R: %f,  pos F: %f,  pos OOV: %f" %
                                         (epoch + 1, pp, pr, pf, poov))
                            writer.write("Epoch: %d,  ppos P: %f,  ppos R: %f,  ppos F: %f,  pos OOV: %f" %
                                         (epoch + 1, pPP, pRR, pFF, 123321))
                        ...
                        print('best_eval_model_path!!!!!!!', best_eval_model_path)
...
def test(args):
    ...
    joint_model = SCwsPos.from_spec(joint_model_checkpoint['spec'], joint_model_checkpoint['state_dict'], args)
    ...
    for example, y_true_item in zip(eval_examples, y_true):
        ...
        if len(y_true_item) != len(sen):
            ...
            sen = sen[:len(y_true_item)]
        sentence_all.append(sen)
    (wp, wr, wf), (pp, pr, pf) , (_,_,_)= pos_evaluate_word_PRF(y_pred_all, y_true_all)
    woov, poov = pos_evaluate_OOV(y_pred, y_true, sentence_all, word2id)
...
def main():
    ...
    parser.add_argument('--encoder_type', type=str, default='transformer', help="")
